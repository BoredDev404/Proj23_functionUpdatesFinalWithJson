...
const LifeTrackerApp = {
    ...
    setupImportExport() {
        ...
        document.getElementById('cancelImportExport')?.addEventListener('click', () => {
            this.hideModal('importExportModal');
        });
    ...
    async exportAllData() {
        try {
            const exportData = {
                ...
                metadata: {
                    totalEntries: {
                        dopamine: (await db.dopamineEntries.toArray()).length,
                        habits: (await db.hygieneHabits.toArray()).length,
                        completions: (await db.hygieneCompletions.toArray()).length,
                        mood: (await db.moodEntries.toArray()).length
                    }
                }
            };
            ...
            this.showImportStatus('success', '✅ Data exported successfully! You can now edit the JSON file.');
            
        } catch (error) {
            console.error('Export failed:', error);
            this.showImportStatus('error', '❌ Export failed: ' + error.message);
        }
    },
    ...
    async handleFileImport(file) {
        ...
        if (!confirm('❌ This action cannot be undone! All current data will be lost.\n\nProceed with import?')) {
            return;
        }
        ...
        try {
            ...
            if (importData.data.hygieneHabits) {
                await db.hygieneHabits.bulkAdd(importData.data.hygieneHabits);
            }
            
            if (importData.data.dopamineEntries) {
                await db.dopamineEntries.bulkAdd(importData.data.dopamineEntries);
            }
            
            if (importData.data.moodEntries) {
                await db.moodEntries.bulkAdd(importData.data.moodEntries);
            }
            
            if (importData.data.hygieneCompletions) {
                await db.hygieneCompletions.bulkAdd(importData.data.hygieneCompletions);
            }
            
            if (importData.data.dailyCompletion) {
                await db.dailyCompletion.bulkAdd(importData.data.dailyCompletion);
            }
            
            this.showImportStatus('success', '✅ Data imported successfully!');
            
            setTimeout(() => {
                this.hideModal('importExportModal');
                this.renderAllPages();
                alert('✨ Import successful! All pages have been refreshed.');
            }, 1500);
    ...
    showImportStatus(type, message) {
        ...
        statusEl.className = `email-status email-${type}`;
...
const additionalStyles = `
    ...
    .email-loading {
        ...
        border: 1px solid var(--ig-warning);
        display: block !important;
    }
